...
class AccommodationSummary extends Component {
  ...
  getAccommodation = index => {
    ...
    this.props.navigation.navigate("ListAccomodation", {
      Parameter: {
        Index: index,
        CityId: cityId,
        StartDate: StartDate,
  ...
  handleCity = async index => {
    await this.props.navigation.navigate("ListCity", {
  ...
  handleAirport = (data, index, type) => {
    ...
    this.props.navigation.navigate("ListAirport", {
      data: data,
      index: index,
  ...
  onSelectFirstCity = async (index, city) => {
    ...
    //this.props.dispatch(set_returns_itenerary(newReturns));
    //this.props.dispatch(set_summary_program(data.SP));
    this.setState({ loading: false });
  };
  ...
  onSelectAirport = async (data, index, airport) => {
    ...
    if (index != null) {
      ...
      if (this.state.airportType == "firstDeparture") {
        ...
        this.props.setDeparturesItineraryAction(arrayDepartures);
        // await this.props.dispatch(set_departures_itenerary(arrayDepartures));
        this.setState({ loading: false });
      } else if (this.state.airportType == "firstArrival") {
        let placesByRegion = await this.getAirportbyRegion(airport.Region.Id);
        let city = await this.props.cityList.find(
        ...
        // await this.props.dispatch(set_departures_itenerary(arrayDepartures));
        if (
          !isAccessibleAirport(airport, placesByRegion, this.state.oldAirport)
        ) {
        ...
        this.props.setReturnsItineraryAction(arrayReturns);
        // await this.props.dispatch(set_returns_itenerary(arrayReturns));
        this.setState({ loading: false });
      } else {
        ArrDep.TransferType = "Movement_arrival";
  ...
  handleTicket = (
    ...
    if (!errorDeparture) {
      ...
      this.props.navigation.navigate("ListAirlineTicket", {
                ...
                ? splitFlightTransit(this.state.Returns, 2).map((item, i) => {
                    return (
                      <CardFlight
                        ...
                        onPressArrivalDate={() =>
                          this.setState({ isDateRetArrivalPickerVisible: true })
                        }
                        onPressArrivalTime={() =>
                          this.setState({ isTimeRetArrivalPickerVisible: true })
                        }
                        onPressDepartureTime={() =>
                          this.setState({
